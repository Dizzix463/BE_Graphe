...
public abstract class AllShortestPathAlgorithmTest {

    public abstract ShortestPathAlgorithm launchShortestPathAlgorithm(ShortestPathData data);
    ...
    public static Collection<Object> data() throws Exception {
        ...
        ShortestPathData originEqualsDestinationTestData = new ShortestPathData(graph, graph.get(15), graph.get(15),
        ...
        ShortestPathSolution originEqualsDestinationSolution = new ShortestPathSolution(originEqualsDestinationTestData,
                Status.OPTIMAL, originEqualsDestinationPath);

        /* Haute-Garonne unreachable destination */
        ShortestPathData unreachableDestinationData = new ShortestPathData(graph, graph.get(120349), graph.get(120351),
                getAllFilters().get(FilterType.ALL_ROADS_AND_LENGTH.getValue()));
        ShortestPathSolution unreachableDestinationPath = new ShortestPathSolution(unreachableDestinationData,
                Status.INFEASIBLE);

        objects.add(
                new TestParameters(roadsForCarsSolution.getInputData(), roadsForCarsSolution, "Only roads for cars"));
        ...
        objects.add(
                new TestParameters(unreachableDestinationData, unreachableDestinationPath, "Unreachable destination"));
    ...
    private static ShortestPathSolution createRegularTestSolution(Graph graph, String solutionPathFileName,
        ...
        ShortestPathData roadsForCarsSolutionData = new ShortestPathData(graph, roadsForCarsSolutionPath.getOrigin(),
                roadsForCarsSolutionPath.getDestination(), getAllFilters().get(filter.getValue()));
    ...
    @Before
    public void init() {
        ShortestPathAlgorithm algorithm = launchShortestPathAlgorithm(parameters.data);
    ...
    public void isPathCostCorrect() {
        ...
        assertEquals(parameters.solution.getPath().getMinimumTravelTime(),
                computedSolution.getPath().getMinimumTravelTime(), 1e-6);
    }
